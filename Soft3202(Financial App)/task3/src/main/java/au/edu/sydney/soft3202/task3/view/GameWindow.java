...
public class GameWindow {
    ...
    public GameWindow(GameBoard model,String userName) {
        this.model = model;
    ...
    private void serialiseAction() {
        ...
        dialog.setTitle("Serialise Game");
        dialog.setHeaderText("Enter a name for the serialised game");
        Optional<String> result = dialog.showAndWait();
        if (result.isPresent()) {

            if (Database.checkGameName(dialog.getResult())) {
                System.out.println(1);
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Serialisation Error");
                alert.setHeaderText("A game with that name already exists! Do you want to overwrite it?");
                ButtonType overwrite = new ButtonType("Overwrite");
                ButtonType cancel = new ButtonType("Cancel", ButtonBar.ButtonData.CANCEL_CLOSE);
                alert.getButtonTypes().setAll(overwrite, cancel);

                Optional<ButtonType> result1 = alert.showAndWait();
                if (result1.isPresent() && result1.get() == overwrite) {
                    Database.deleteGame(dialog.getResult());
                    String serialisation = model.serialise(dialog.getResult());
                }
            }
...
//        textInput.showAndWait();
    ...
    private void deserialiseAction() {
        ...
        dialog.setTitle("Deserialise Game");
        dialog.setHeaderText("Enter the serialised game");
        dialog.showAndWait();




        String input = Database.SelectData(username, dialog.getResult());
        String serialisation = input;
